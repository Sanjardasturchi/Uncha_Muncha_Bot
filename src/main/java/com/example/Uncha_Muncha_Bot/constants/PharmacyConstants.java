package com.example.Uncha_Muncha_Bot.constants;

public interface PharmacyConstants {
    String PHARMACY="Pharmacy menu";
    String PHARMACY_FOR_PEOPLE="Pharmacy for people";
    String PHARMACY_FOR_ANIMALS="Pharmacy for animals";

    // ========================ADMIN===================
    String CREATE="Create new pharmacy";
    String ADD_MEDIA="Add media to pharmacy";
    String MAKE_BLOCK="Make block pharmacy";
    String MAKE_UNBLOCK="Make unblock pharmacy";
    String GET_ALL="Get all pharmacy";
    String GET_BY_ID="Get pharmacy by id";

    String ACCEPT_TO_CREATE="Accept to create new pharmacy";
    String CHOOSE_PHARMACY_TYPE="Choose pharmacy type";
    String CHOOSE_PHARMACY_WORKING_START_TIME="Choose pharmacy working start time";
    String CHOOSE_PHARMACY_WORKING_END_TIME="Choose pharmacy working end time";
    String ENTERING_OWNER_USERNAME="Entering owner username";
    String ENTERING_PHARMACY_PHONE_NUMBER="Entering pharmacy phone number";
    String ENTERING_PHARMACY_NAME="Entering pharmacy name";
    String ENTERING_INFO_ABOUT_PHARMACY="Entering info about pharmacy";
    String ENTERING_INFO_ABOUT_PHARMACY_UZ="Entering info about pharmacy uz";
    String ENTERING_INFO_ABOUT_PHARMACY_TR="Entering info about pharmacy tr";
    String ENTERING_INFO_ABOUT_PHARMACY_RU="Entering info about pharmacy ru";
    String ENTERING_PHARMACY_LOCATION_EN="Entering pharmacy location en";
    String ACCEPT_TO_FINISH_CREATING="Accept to finish creating new pharmacy";

    String SENDING_PHARMACY_ID_FOR_ADD_MEDIA="Entering pharmacy id for add media";
    String SENDING_PHARMACY_ID_FOR_MAKE_BLOCK="Entering pharmacy id for make block";
    String SENDING_PHARMACY_ID_FOR_MAKE_UNBLOCK="Entering pharmacy id for make unblock";
    String SENDING_PHARMACY_ID_FOR_GET="Entering pharmacy id for get";

    String SENDING_MEDIA="Entering Media";
//    ======================USER======================
    String SEND_LOCATION_FOR_PEOPLE_PHARMACY="Send location for people pharmacy";
    String SEND_LOCATION_FOR_ANIMALS_PHARMACY="Send location for people pharmacy";


}
